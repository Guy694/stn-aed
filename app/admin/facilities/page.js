...
import {
  Plus, Pencil, Trash2, Search, X, Heart,
...
export default function AdminFacilitiesPage() {
  ...
  const handleSave = (saved) => {
    ...
    showToast(modal.facility?.id ? 'แก้ไขข้อมูลสำเร็จ' : 'เพิ่มหน่วยบริการสำเร็จ');
  };
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 shadow-sm">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500 mt-0.5">เพิ่ม แก้ไข ลบข้อมูลหน่วยบริการสาธารณสุข</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setModal({ open: true, facility: null })} className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition-all hover:bg-emerald-500">
                    ...
                    {filtered.map((f) => (
                      <tr key={f.id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center flex-shrink-0"><Building2 className="w-3.5 h-3.5 text-white" /></div>
